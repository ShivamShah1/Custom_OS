...
/* internal representation of directories and files */
...
/*
    fat16_extention block header in the boot section for kernel to read the filesystem
*/
...
/*
    normal fat16 block reader for kernel
*/
struct fat_header{
...
/*
    fat16 filesystem infomation block of a file
*/
struct fat_directory_item{
    ...
    uint16_t lower_16_bits_first_cluster;
    uint32_t filesize;
} __attribute__((packed));

/*
    blocks allocated to a file information
*/
...
/*
    private files data information
*/
struct fat_private{
    struct fat_h header;
...
/*
    entry block functions info due to rename
*/
...
/*
    initialization of the file in fat16
*/
...
/*
    initialization of the private file information
*/
...
/*
    providing the number of block sectors to the file
*/
...
/*
    providing all the necessary data and information to the file for the creation
    with all the basic information and providing the needed space.
*/
int fat16_get_items_for_directory(struct disk* disk, uint32_t directory_start_sector){
    struct fat_directory_item item;
...
/*
    getting the proper path from root directory to the file
*/
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory){
    ...
    int total_items = fat16_get_items_for_directory(disk, root_dir_sector_pos);
...
/*
    successful creaton of the file and inserting it in the filesystem
*/
int fat16_resolve(struct disk* disk){
    int res = 0;
...
out:
    ...
    /* returning zero after successful creation of filesystem so that we can insert filesystem to the disk */
    return res;
}

/*
    to get the proper string of the path
*/
...
/*
    to get the full relative filename from the path

    suppose the name of the file is test and extention is txt, then
    as it will be stored in file header seperately, using this we will get them in single string
    "test.txt". 
*/
...
/*
    struct for cloning the directory and items 
*/
struct fat_item* fat16_clone_directory_item(struct fat_directory_item* item, int size){
...
/*
    getting the first sector or the entry sector or cluster of the directory
*/
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset){
    ...
    for(int i=0; i<clusters_ahead; i++){
        ...
        if(entry == 0xFFf8 || entry == 0xFFFF){
            /* we are at teh last entry in the file */
            res = -EIO;
            goto out;
        }
        ...
        /* are there 0(NULL) or no sectors */
        /* due to currouption of memory there are no sectors or unable to read */
        if(entry == 0x00){
            res = -EIO;
            goto out;
        }
...
/*
    reading from the internal part from the stream
*/
static int fat16_read_internal_from_stream(struct disk* disk, struct disk_stream* stream, int cluster, int offset, int total, void* out){
    ...
    /* we need to make sure that it does not go out of space */
    /* for this project this is okay as we will not use big files and sectors */
    if(total > 0){
...
/*
    reading the internal of the fat16 directory.
    It will help to read the whole cluster as a single cluster
    which is not. 
*/
...
/*
    geting the firest cluster of the directory
*/
static uint32_t fat16_get_first_cluster(struct fat_directory_item* item){
    return (item->high_16_bits_first_cluster << 16) | item->lower_16_bits_first_cluster;
}

/*
    converting the fat16 cluster into the respective sector
*/
...
/*
    freeing the memeory which was used in loading the directory
*/
...
struct fat_directory* fat16_load_fat_directory(struct disk* disk, struct fat_directory_item* item){
    ...
    /* checking if we are loading directroies only */
    ...
    /* this is the cluster where all the directories are listed */
    ...
    directory->total = total_items;
    int directory_size = directory->total + sizeof(struct fat_item);
    /* loading entire directory into items */
    ...
    /* this is to find the next cluster to read of the file */
    /* so that we do not need to do the calculation all the time */
...
/*
    struct for new items in the old directory item
*/
struct fat_item* fat16_new_fat_item_for_directory_item(struct disk* disk, struct fat_directory_item* item){
    ...
    /* here we are cloning this item directory as we are allocating the memory using  kzalloc */
    /* there is a chance of currupting the space as this might get freed and issued to other funcitons */
...
/*
    struct to find the item in directory in the memory locaiton/path provided.
*/
...
/*
    providing the struct for the fat16 entries from the directory store in the memory
    and initializing this with them
*/
struct fat_item* fat16_get_directory_entry(struct disk* disk, struct path_part* path){
    ...
    /* the item is not present that that location of memory */
    if(!root_item){
        goto out;
    }
    ...
    while(next_part != 0){
        /* if the path has more path or directory */
...
void* fat16_open(struct disk* disk, struct path_part* path, FILE_MODE mode){
    ...
    /* allocating the required mem for the descriptor */
    ...
    /* this will look in the path for that file and return that to this variable */
    ...
    /* at the open of the file, the pointer should pointer to the first char */
    descriptor->pos = 0;
...
/*
    providing the stats of the fat16 block of file
*/
int fat16_stat(struct disk* disk, void* private, struct file_stat* stat){
    int res = 0;
...
/*
    freeing the fat16 file descriptor from fclose
*/
